...
def locate_nearest_normal_N1(x_data_rows_cutN1, y_data_rows_cutN1, z_data_rows_cutN1,probe_z):
    #input negative z
    #write a function to determine whether column index needs to be 0 or -1.
    endx=[]
    endy=[]
    endz=[]
    [endz.append(z_data_rows_cutN1[i][-1]) for i in range(len(z_data_rows_cutN1))]
    endz_np=np.array(endz)
    diff=np.abs(endz_np-probe_z)
    index,=np.where(diff==min(diff))
    row_index=index[0]
...
x_data_rows_uncutN1, y_data_rows_uncutN1, z_data_rows_uncutN1=tile_rows(filenameN1)
radius_surface, theta_surface, z_surface=cartesian_to_cylindrical_position_coordinates(filenameN1)

min_theta=-15*np.pi/180
max_theta=0
tile_gap=2e-3
x_data_rows_cutN1, y_data_rows_cutN1, z_data_rows_cutN1=tile_rows_cut(filenameN1,min_theta,max_theta)

probe_z=-2.05
row_indexN1,column_indexN1=locate_nearest_normal_N1(x_data_rows_cutN1, y_data_rows_cutN1, z_data_rows_cutN1,probe_z)
#i do not think surface-normal is constant on N1 tile in cylindrical coordinates.
point1N1, point2N1, point3N1=three_points(x_data_rows_cutN1, y_data_rows_cutN1, z_data_rows_cutN1, row_indexN1,column_indexN1)
surface_normal_xyzN1=surface_normal_xyz_fn(point1N1,point2N1,point3N1)
cylindrical_unit_vectorsN1=calculate_cylindrical_unit_vectors(point1N1)
surface_normal_rTzN1=cylindrical_transform_surface_normal_xyz(surface_normal_xyzN1,cylindrical_unit_vectorsN1,'N1')
loc='lower'
if surface_normal_rTzN1[0]>0:
    surface_normal_rTzN1=surface_normal_rTzN1*-1

if loc=='upper':
    surface_normal_rTzN1=np.array([surface_normal_rTzN1[0] , surface_normal_rTzN1N1[1] , -1*surface_normal_rTzN1[2]  ])
if loc=='lower':
    surface_normal_rTzN1=np.array([surface_normal_rTzN1[0] , surface_normal_rTzN1[1] , surface_normal_rTzN1[2]  ])

print(surface_normal_rTzN1)

vector_dataN1=create_vector_data_cyl(surface_normal_rTzN1, cylindrical_unit_vectorsN1, point1N1)





rz_normalN1=tile_unit_components_rz(surface_normal_rTzN1)
probe_angle_wrt_rz_normal=3.5*np.pi/180
scalingN1=np.cos(probe_angle_wrt_rz_normal)/(rz_normalN1[0]**2+rz_normalN1[2]**2)
probe_normal_thetaN1=(1-(scalingN1**2)*(rz_normalN1[0]**2+rz_normalN1[2]**2))**0.5
probe_surface_normal_rTzN1=np.array([scalingN1*rz_normalN1[0] , probe_normal_thetaN1 , scalingN1*rz_normalN1[2]  ])


vector_probedataN1=create_vector_data_cyl(probe_surface_normal_rTzN1, cylindrical_unit_vectorsN1, point1N1)

mag_probe_surface_normal_rtzN1=(probe_surface_normal_rTzN1[0]**2+probe_surface_normal_rTzN1[1]**2+probe_surface_normal_rTzN1[2]**2)**0.5
print(mag_probe_surface_normal_rtzN1)
...
for i in range(len(x_data_rows_uncutN1)):
    plt.plot(x_data_rows_uncutN1[i],y_data_rows_uncutN1[i],z_data_rows_uncutN1[i],'o',color='r',linestyle='')
for i in range(len(x_data_rows_cutN1)):
    plt.plot(x_data_rows_cutN1[i],y_data_rows_cutN1[i],z_data_rows_cutN1[i],'o',color='m',linestyle='')
plt.plot(vector_dataN1[0],vector_dataN1[1],vector_dataN1[2])
plt.plot(vector_probedataN1[0],vector_probedataN1[1],vector_probedataN1[2],'r')
plt.show()
