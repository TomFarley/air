...
;
   ...
   if (nalpha gt 1) then begin
      ...
      s=fltarr(nelem)
      s(0)=alphaconst(1)  
      item = ida_create(fid, fprefix+'_ALPHACONST_isp_ELM', fshot)
      ok = ida_set_structure(item, ida_dct, 1, ida_d4+ida_real, xtsams=nelem)
      ok = ida_set_dinfo(item, 0.0, 1.0, 0, 1.0, 0.0, 'alpha', 'alpha')
      ok = ida_set_data(item, s, ida_d4+ida_real+ida_valu)
   endif
...
; time evolution of maxima/minima
   ...
   err = GC_PUTIT(fid, shotnr, fprefix+'_minpower_density_isp', 'MWm-2',$
               tout, qmin(sti:eti))
;
   err = GC_PUTIT(fid, shotnr, fprefix+'_pkpower_density_isp', 'MWm-2',$
                       tout, qmax(sti:eti))
;
   if (nalpha gt 1) then begin
   err = GC_PUTIT(fid, shotnr, fprefix+'_minpow_dens_isp_ELM', 'MWm-2',$
                       tout, qmin_ELM(sti:eti))
;
      err = GC_PUTIT(fid, shotnr, fprefix+'_pkpow_dens_isp_ELM', 'MWm-2',$
                       tout, qmax_ELM(sti:eti))
   endif
;
;
   err = GC_PUTIT(fid, shotnr, fprefix+'_peakpower_pos_isp', 'm',$
                       tout, rqmax(sti:eti))
;
...
;
   err = put_trace_dt(fid,shotnr,fprefix+'_qprofile_isp','MW m-2', $ 
...
;
   ...
   if (nalpha gt 1) then begin
;
      err = put_trace_dt(fid,shotnr,fprefix+'_qprofile_isp_ELM','MW m-2', $
...
;FL 24/11/2003: adding PTOT and ETOT trace
    err = GC_PUTIT(fid, shotnr, fprefix+'_ptot_isp', 'MW',$ 
                   tout, ptot(sti:eti))
    ...
    if (nalpha gt 1) then begin
       err = GC_PUTIT(fid, shotnr, fprefix+'_ptot_isp_ELM', 'MW',$
                   tout, ptot_ELM(sti:eti))
       err = GC_PUTIT(fid, shotnr, fprefix+'_etot_isp_ELM', 'kJ',$
                   tout, 1.e3*etot_ELM(sti:eti))
       err = GC_PUTIT(fid, shotnr, fprefix+'_etotsum_isp_ELM', 'kJ',$
