...
class EfitData():
    def __init__(self, include_converged_times=True, include_vacuum_times=True,
        ...
        if not read_archive and not read_eqdsk:
            self._read_data(filename, include_converged_times=include_converged_times,
                            include_vacuum_times=include_vacuum_times,
                            include_non_converged_times=include_non_converged_times,
                            include_fatal_times=include_fatal_times,
            ...
            all_data = tok_module.read_tok_data(shot, include_converged_times=include_converged_times,
                                                include_vacuum_times=include_vacuum_times,
                                                include_non_converged_times=include_non_converged_times,
                                                include_fatal_times=include_fatal_times,
                                                subsample=subsample, sample_count=sample_count, time_min=time_min,
                                                time_max=time_max)
            self.data = all_data
        elif read_eqdsk:
            self._read_eqdsk(geqdsk_indexfile, subsample=subsample, sample_count=sample_count, time_min=time_min,
                             time_max=time_max, n_pf=n_pf)
    ...
    def _read_data(self, filename, include_converged_times=True, include_vacuum_times=True,
        ...
        if efit_time_indices.size == 0:
            if self.debug > 0:
                ...
                print("          You requested to include converged? {} vacuum? {} non-converged? {} fatal? {}".format(
                    include_converged_times,
        ...
        print(
            "<<INFO>> efitplot will process: {}  time-slices in the time range: {}<t<{}".format(len(efit_time_indices),
                                                                                                mintime, maxtime))
        ...
        self.data["constraints"]["plasmaCurrent"] = self._read_group(filename, "/input/constraints/plasmaCurrent",
                                                                     itimes=efit_time_indices)
        self.data["constraints"]["pfCircuits"] = self._read_group(filename, "/input/constraints/pfCircuits",
                                                                  itimes=efit_time_indices)
        self.data["constraints"]["fluxLoops"] = self._read_group(filename, "/input/constraints/fluxLoops",
                                                                 itimes=efit_time_indices)
        self.data["constraints"]["magneticProbes"] = self._read_group(filename, "/input/constraints/magneticProbes",
                                                                      itimes=efit_time_indices)
        self.data["constraints"]["mse"] = self._read_group(filename, "/input/constraints/mse", itimes=efit_time_indices)
        self.data["constraints"]["lineIntegratedDensities"] = self._read_group(filename,
                                                                               "/input/constraints/lineIntegratedDensities",
                                                                               itimes=efit_time_indices)
        self.data["constraints"]["discreteDensities"] = self._read_group(filename,
                                                                         "/input/constraints/discreteDensities",
                                                                         itimes=efit_time_indices)
        self.data["constraints"]["pressures"] = self._read_group(filename, "/input/constraints/pressures",
                                                                 itimes=efit_time_indices)
        self.data["constraints"]["faradayRotationChannels"] = self._read_group(filename,
                                                                               "/input/constraints/faradayRotationChannels",
                                                                               itimes=efit_time_indices)
        self.data["constraints"]["diamagneticFlux"] = self._read_group(filename, "/input/constraints/diamagneticFlux",
                                                                       itimes=efit_time_indices)
        self.data["constraints"]["boundaries"] = self._read_group(filename, "/input/constraints/boundaries",
                                                                  itimes=efit_time_indices)
        ...
        self.data["fluxFunctionProfiles"] = self._read_group(filename, "/output/fluxFunctionProfiles",
                                                             itimes=efit_time_indices)
        ...
        # --------------------------
        self.data["numerics"] = {}
        self.data["numerics"]["numericalControls"] = self._read_group(filename, "/input/numericalControls",
                                                                      itimes=efit_time_indices)
        self.data["numerics"]["pp"] = self._read_group(filename, "/input/numericalControls/pp",
                                                       itimes=efit_time_indices)
        self.data["numerics"]["ff"] = self._read_group(filename, "/input/numericalControls/ff",
                                                       itimes=efit_time_indices)
        self.data["numerics"]["ww"] = self._read_group(filename, "/input/numericalControls/ww",
                                                       itimes=efit_time_indices)
        self.data["numerics"]["ne"] = self._read_group(filename, "/input/numericalControls/ne",
                                                       itimes=efit_time_indices)
        self.data["numerics"]["numericalDetails"] = self._read_group(filename, "/output/numericalDetails",
                                                                     itimes=efit_time_indices)
        ...
        if self.data["constraints"]["boundaries"] is not None:
            ...
            self.data["constraints"]["boundaries"] = self.data["constraints"]["boundaries"].assign(
                computedSep=(['time', 'boundariesDim'], bound_computed.values.reshape((self.data["top"].time.size, 1))))
    ...
    def _read_bvacradiusproduct(self, filename, itimes):
        ...
        try:
            ...
            if self.debug > 0:
                print(
                    "<<WARNING>> Could not find output group or variable in output group: {} in netcdf file {}".format(
                        err, filename))
        ...
        for ind_time, (time, geqdsk, aeqdsk) in enumerate(zip(all_times, all_geqdsk, all_aeqdsk)):
            ...
            if self.data["regularGrid"] is None:
                self.data["regularGrid"] = xr.Dataset({'rMin': (['unityDim'], [geqdsk_data.data['rleft']]),
                                                       'rMax': (['unityDim'],
                                                                [geqdsk_data.data['rleft'] + geqdsk_data.data['rdim']]),
                                                       'zMin': (['unityDim'], [
                                                           geqdsk_data.data['zmid'] + geqdsk_data.data['zdim'] / 2.0]),
                                                       'zMax': (['unityDim'], [
                                                           geqdsk_data.data['zmid'] - geqdsk_data.data['zdim'] / 2.0]),
            ...
            if self.data["profiles2D"] is None:
                rgrid = np.arange(geqdsk_data.data['nw']) * geqdsk_data.data['rdim'] / float(
                    geqdsk_data.data['nw'] - 1) + geqdsk_data.data['rleft']
                zgrid = np.arange(geqdsk_data.data['nh']) * geqdsk_data.data['zdim'] / float(
                    geqdsk_data.data['nh'] - 1) + geqdsk_data.data['zmid'] - 0.5 * geqdsk_data.data['zdim']
                ...
                self.data["profiles2D"] = xr.Dataset({'r': (['time', 'rgrid'], np.copy(nan_array_2d)),
                                                      ...
                                                      'poloidalFlux': (
                                                      ['time', 'rgrid', 'zgrid'], np.copy(nan_array_3d)),
            ...
            if geqdsk_data.data['rmaxis'] > 0.0:
                ...
                if self.data["fluxFunctionProfiles"] is None:
                    ...
                    self.data["fluxFunctionProfiles"] = xr.Dataset(
                        {'rBphi': (['time', 'normalizedPoloidalFlux'], np.copy(nan_array_2d)),
                ...
                if self.data["separatrixGeometry"] is None and nbbbs > 0:
                    ...
                    nan_array_2d[:] = np.nan
                    self.data["separatrixGeometry"] = xr.Dataset(
                        {'rBoundary': (['time', 'boundaryDims'], np.copy(nan_array_2d)),
                ...
                self.data["profiles2D"].poloidalFlux.values[ind_time, :, :] = flip_psi_factor * np.transpose(
                    geqdsk_data.data['psirz'])
                ...
                self.data["globalParameters"].bvacRmag.values[ind_time] = geqdsk_data.data['rmaxis'] * geqdsk_data.data[
                    'bcentr'] / geqdsk_data.data['rcentr']
                ...
                self.data["fluxFunctionProfiles"].ffPrime.values[ind_time, :] = flip_psi_factor * geqdsk_data.data[
                    'ffprime']

                self.data["fluxFunctionProfiles"].staticPPrime.values[ind_time, :] = flip_psi_factor * geqdsk_data.data[
                    'pprime']
                self.data["fluxFunctionProfiles"].q.values[ind_time, :] = flip_psi_factor * geqdsk_data.data['qpsi']

                if geqdsk_data.data['nbbbs'] > 0:
                    self.data["separatrixGeometry"].rBoundary.values[ind_time, 0:geqdsk_data.data['nbbbs']] = \
                    geqdsk_data.data['rbbbs']
                    self.data["separatrixGeometry"].zBoundary.values[ind_time, 0:geqdsk_data.data['nbbbs']] = \
                    geqdsk_data.data['zbbbs']
            ...
            if aeqdsk is not None:
                ...
                if self.data["separatrixGeometry"] is not None and 'rGeom' not in self.data["separatrixGeometry"]:
                    self.data["separatrixGeometry"] = self.data["separatrixGeometry"].assign(
                        rGeom=(['time'], np.copy(nan_array)))
                    self.data["separatrixGeometry"] = self.data["separatrixGeometry"].assign(
                        zGeom=(['time'], np.copy(nan_array)))
                    self.data["separatrixGeometry"] = self.data["separatrixGeometry"].assign(
                        minorRadius=(['time'], np.copy(nan_array)))
                    self.data["separatrixGeometry"] = self.data["separatrixGeometry"].assign(
                        elongation=(['time'], np.copy(nan_array)))
                    self.data["separatrixGeometry"] = self.data["separatrixGeometry"].assign(
                        upperTriangularity=(['time'], np.copy(nan_array)))
                    self.data["separatrixGeometry"] = self.data["separatrixGeometry"].assign(
                        lowerTriangularity=(['time'], np.copy(nan_array)))
                    self.data["separatrixGeometry"] = self.data["separatrixGeometry"].assign(
                        dndXpointCount=(['time'], np.copy(nan_array)))
                    self.data["separatrixGeometry"] = self.data["separatrixGeometry"].assign(
                        dndXpoint1R=(['time'], np.copy(nan_array)))
                    self.data["separatrixGeometry"] = self.data["separatrixGeometry"].assign(
                        dndXpoint1Z=(['time'], np.copy(nan_array)))
                    self.data["separatrixGeometry"] = self.data["separatrixGeometry"].assign(
                        dndXpoint2R=(['time'], np.copy(nan_array)))
                    self.data["separatrixGeometry"] = self.data["separatrixGeometry"].assign(
                        dndXpoint2Z=(['time'], np.copy(nan_array)))

                    self.data["globalParameters"] = self.data["globalParameters"].assign(
                        plasmaVolume=(['time'], np.copy(nan_array)))
                    self.data["globalParameters"] = self.data["globalParameters"].assign(
                        currentCentroid=(['time'], np.copy(coord_nan_array)))
                    self.data["globalParameters"] = self.data["globalParameters"].assign(
                        betat=(['time'], np.copy(nan_array)))
                    self.data["globalParameters"] = self.data["globalParameters"].assign(
                        betap=(['time'], np.copy(nan_array)))
                    self.data["globalParameters"] = self.data["globalParameters"].assign(
                        li=(['time'], np.copy(nan_array)))
                    self.data["globalParameters"] = self.data["globalParameters"].assign(
                        q95=(['time'], np.copy(nan_array)))
                    self.data["globalParameters"] = self.data["globalParameters"].assign(
                        s1=(['time'], np.copy(nan_array)))
                    self.data["globalParameters"] = self.data["globalParameters"].assign(
                        s2=(['time'], np.copy(nan_array)))
                    self.data["globalParameters"] = self.data["globalParameters"].assign(
                        s3=(['time'], np.copy(nan_array)))
                    self.data["globalParameters"] = self.data["globalParameters"].assign(
                        poloidalArea=(['time'], np.copy(nan_array)))
                    self.data["globalParameters"] = self.data["globalParameters"].assign(
                        plasmaEnergy=(['time'], np.copy(nan_array)))
                    self.data["globalParameters"] = self.data["globalParameters"].assign(
                        q0=(['time'], np.copy(nan_array)))
                    self.data["globalParameters"] = self.data["globalParameters"].assign(
                        diamagneticFlux=(['time'], np.copy(nan_array)))
                    self.data["globalParameters"] = self.data["globalParameters"].assign(
                        alpha=(['time'], np.copy(nan_array)))
                    self.data["globalParameters"] = self.data["globalParameters"].assign(
                        rt=(['time'], np.copy(nan_array)))
                    self.data["globalParameters"] = self.data["globalParameters"].assign(
                        bphiRmag=(['time'], np.copy(nan_array)))
                    self.data["globalParameters"] = self.data["globalParameters"].assign(
                        q1Radius=(['time'], np.copy(nan_array)))
                    self.data["globalParameters"] = self.data["globalParameters"].assign(
                        q2Radius=(['time'], np.copy(nan_array)))
                    self.data["globalParameters"] = self.data["globalParameters"].assign(
                        q3Radius=(['time'], np.copy(nan_array)))
                    self.data["globalParameters"] = self.data["globalParameters"].assign(
                        betapd=(['time'], np.copy(nan_array)))
                ...
                if aeqdsk_data.data['rout'] > 0.0:
                    ...
                    if self.data["separatrixGeometry"].dndXpointCount.values[ind_time] > 0:
                        self.data["separatrixGeometry"].dndXpoint1R.values[ind_time] = aeqdsk_data.data[
                                                                                           'rseps0'] / 100.0
                        self.data["separatrixGeometry"].dndXpoint1Z.values[ind_time] = aeqdsk_data.data[
                                                                                           'zseps0'] / 100.0

                    if self.data["separatrixGeometry"].dndXpointCount.values[ind_time] > 1:
                        self.data["separatrixGeometry"].dndXpoint2R.values[ind_time] = aeqdsk_data.data[
                                                                                           'rseps1'] / 100.0
                        self.data["separatrixGeometry"].dndXpoint2Z.values[ind_time] = aeqdsk_data.data[
                                                                                           'zseps1'] / 100.0

                    self.data["globalParameters"].currentCentroid.values[ind_time]['R'] = aeqdsk_data.data[
                                                                                              'rcurrt'] / 100.0
                    self.data["globalParameters"].currentCentroid.values[ind_time]['Z'] = aeqdsk_data.data[
                                                                                              'zcurrt'] / 100.0
                ...
                if self.data["constraints"]["fluxLoops"] is None and nsilop > 0:
                    ...
                    self.data["constraints"]["fluxLoops"] = xr.Dataset(
                        {'computed': (['time', 'fluxLoopDim'], np.copy(nan_array_2d)),
                ...
                if self.data["constraints"]["magneticProbes"] is None and nmagpr > 0:
                    ...
                    self.data["constraints"]["magneticProbes"] = xr.Dataset(
                        {'computed': (['time', 'magneticProbeDim'], np.copy(nan_array_2d)),
                ...
                if self.data["constraints"]["pfCircuits"] is None and nfcoil > 0:
                    ...
                    self.data["constraints"]["pfCircuits"] = xr.Dataset(
                        {'computed': (['time', 'pfCircuitsDim'], np.copy(nan_array_2d)),
                ...
                if nsilop > 0:
                    self.data["constraints"]["fluxLoops"].computed.values[ind_time, :] = 2 * np.pi * aeqdsk_data.data[
                        'csilop']
                if nmagpr > 0:
                    self.data["constraints"]["magneticProbes"].computed.values[ind_time, :] = aeqdsk_data.data['cmpr2']
                if necurrt > 0:
                    self.data["constraints"]["pfCircuits"].computed.values[ind_time, :necurrt] = aeqdsk_data.data[
                        'eccurt']
                if nfcoil > 0:
                    self.data["constraints"]["pfCircuits"].computed.values[ind_time, necurrt:] = aeqdsk_data.data[
                        'ccbrsp']
    ...
    def calc_chi2(self):
        for constraint in self.data["constraints"].keys():
            ...
            if constraint != "boundaries":
                ...
                self.data["constraints"][constraint] = chi2calc(self.data["constraints"][constraint],
                                                                computedname='computedSep', targetname='rCoords',
                                                                sigmaname='rSigmas')
    ...
    def calc_bfield(self):
        ...
        # B poloidal
        ip_sign = 1
        if np.sum(np.isnan(self.data["globalParameters"].plasmaCurrent.values)) != len(
                self.data["globalParameters"].plasmaCurrent.values):
    ...
    def save_eqdsk(self, time_index, savedir, chease=False, rCentr=1.0, limloc=None, snd_psin_thresh=0.03, afile=True):
        ...
        with open("{}/{}".format(dirpath, geqdsk_filename), 'w') as fp:
            title = 'EFIT++   ' + str(time.strftime("%x")) + '#' + str(pulseNumber) + '-' + str(
                int(time_value * 1000)) + 'ms'
            ...
            print("% .9e% .9e% .9e% .9e% .9e" % (magneticAxis[0], magneticAxis[1],
                                                 flip_psi_factor *
                                                 self.data["globalParameters"].psiAxis.fillna(0.0).values[time_index],
                                                 flip_psi_factor *
                                                 self.data["globalParameters"].psiBoundary.fillna(0.0).values[
                                                     time_index],
                                                 rGeom * self.data["globalParameters"].bvacRgeom.fillna(0.0).values[
                                                     time_index] / rCentr),
                  file=fp)
            print("% .9e% .9e% .9e% .9e% .9e" % (
            self.data["globalParameters"].plasmaCurrent.fillna(0.0).values[time_index],
            ...
            writeColumns(fp, self.data["fluxFunctionProfiles"].staticPressure.fillna(0.0).values[time_index][:], 5,
                         "% .9e")
            writeColumns(fp,
                         flip_psi_factor * self.data["fluxFunctionProfiles"].ffPrime.fillna(0.0).values[time_index][:],
                         5, "% .9e")
            writeColumns(fp, flip_psi_factor * self.data["fluxFunctionProfiles"].staticPPrime.fillna(0.0).values[
                                                   time_index][:], 5, "% .9e")

            psiefit_t = flip_psi_factor * np.transpose(
                self.data["profiles2D"].poloidalFlux.fillna(0.0).values[time_index][:])
            writeColumns(fp, psiefit_t, 5, "% .9e")
            writeColumns(fp, np.absolute(self.data["fluxFunctionProfiles"].q.fillna(0.0)).values[time_index][:], 5,
                         "% .9e")
        ...
        with open("{}/{}".format(dirpath, aeqdsk_filename), 'w') as fp:
            ...
            fp.write(ff_str3.write([self.data["constraints"]["plasmaCurrent"].computed.values[time_index],
                                    rgeom, zgeom,
                                    100.0 * self.data["separatrixGeometry"].minorRadius.fillna(0.0).values[
                                        time_index]]) + '\n')
            ...
            fp.write(ff_str3.write([self.data["globalParameters"].currentCentroid.values[time_index][0] * 100.0,
                                    ...
                                    qstar,
                                    self.data["globalParameters"].betat.fillna(0.0).values[time_index] * 100.0]) + '\n')
            ...
            ind_bound, = np.where(
                np.isclose(self.data["fluxFunctionProfiles"].normalizedPoloidalFlux.values, 1.0) == True)
            if len(ind_bound) > 0:
            ...
            fp.write(ff_str3.write([terror, elongm,
                                    ...
                                    self.data["globalParameters"].diamagneticFlux.fillna(0.0).values[
                                        time_index]]) + '\n')
            ...
            for pind in range(int(np.ceil(nfcoil / 4.0))):
                fp.write(ff_str3.write(
                    self.data["constraints"]["pfCircuits"].computed.values[time_index, pind * 4:(pind + 1) * 4]) + '\n')
            # eccurt(*)
