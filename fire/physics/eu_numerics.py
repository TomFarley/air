...
def iterate_dims_coord(dat, axis, function, *args, coord=None, newcoord=None, **kwargs):
    ...
    if (coord is not None) and (newcoord is not None):
        ...
        for i in range(nsections):
            ...
            out[i * newlen:(i + 1) * newlen] = function(tmp[i * lenaxis:(i + 1) * lenaxis],
                                                        tmp_coord[i * lenaxis:(i + 1) * lenaxis],
                                                        tmp_newcoord[i * newlen:(i + 1) * newlen], **kwargs)
...
def deriv(y, x, method=2):
    ...
    if (method == 2):
        ...
        for i in looparray:
            ...
            if (i == 1):  # first point
                yprime[i - 1] = y[i - 1] * (x01 + x02) / (x01 * x02) - y[i] * x02 / (x01 * x12) + y[i + 1] * x01 / (
                            x02 * x12)
            elif (i == ny - 2):  # last point
                yprime[i + 1] = -y[i - 1] * x12 / (x01 * x02) + y[i] * x02 / (x01 * x12) - y[i + 1] * (x02 + x12) / (
                            x02 * x12)
...
# ------------------------------------------------------------------------------
def compute_status_flag(dat, sigma, r=None, z=None, minval=None, abs_error_threshold=None, rel_error_threshold=None,
                        rmax=3.9, rmin=0., zmax=2., zmin=-2., rrange=None, mask=None):
    """
