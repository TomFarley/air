...
def ats_to_dict(path, fileats, digital_level_bytes=4, header_marker='4949', first_frame_extra_header_marker='4d53',
                n_frames_read=None):
    ...
    data_length = width*height*digital_level_bytes  # Bytes in one frame image
    digitizer_ID = []
    data = []
    time_of_measurement = []
    frame_counter = []
    ...
    value = hexdata.find(header_marker + first_frame_extra_header_marker)  # extra 4949s occur before 1st frame
    i = 0
    ...
    while len(hexdata) - last > header_length:
        ...
        # NOTE: Frame header comes after frame data: , (image, 4949, header), (image, 4949, header) ...
        if (n_frames_read is None) or (i + 1 <= n_frames_read):
            # To quickly retreive meta data, skip reading frame data
            raw_digital_level = hexdata[last+value-data_length:last+value]
            if len(raw_digital_level) == 0:
                raise ValueError(f'Error reading .ats file')
        ...
        if len(time_of_measurement)<=1:	# the spacing between separators seems constant, and take very long, so I do it once
            value = hexdata[last:].find(header_marker)  #  + '4d53')
